...
/* ================================================================
   Resultaatkaart per Elvedes SKU, met verpakkingsvarianten als chips
   ================================================================ */
function ResultCard({
  result,
  matchLabel,
  ...
  const { sku, varianten, padcodeBases, oemArtikelnummers } = result;
  const [selected, setSelected] = useState(0);
  const variant: DescVariant | undefined = varianten[selected];
  ...
  return (
    <article className="card" aria-label={`Elvedes ${sku}`}>
      ...
      {varianten.length > 1 && (
        <div className="card__chips" role="group" aria-label="Verpakkingsvarianten">
          {varianten.map((v, i) => (
            <button
              key={v.artikelnummer}
              type="button"
            ...
            >
              {v.artikelnummer === sku ? "Standaard" : v.artikelnummer.slice(sku.length + 1)}
            </button>
          ))}
        </div>
      )}
        ...
        <div className="searchresults__block">
          ...
          <div className="results">
            {skuResults.map(({ hit, result }) => (
              <ResultCard key={hit.sku} result={result} matchLabel={hit.matchLabel} showFits />
            ))}
          </div>
        </div>
      )}
